...
<type>whole</type></note></measure></part></score-partwise>''';
  ...
  testWidgets('Bar numbers toggle drives single-part numbering via the overlay',
      (tester) async {
    await pump(tester);
    // A note so the notation overlay has element regions to label.
    await tester.sendKeyEvent(LogicalKeyboardKey.keyC);
    await tester.pump();

    MultiSystemView view() =>
        tester.widget<MultiSystemView>(find.byType(MultiSystemView));
    dynamic overlay() => tester
    ...
    // Off in both places to start.
    expect(view().showMeasureNumbers, isFalse);
    expect(overlay()?.showBarNumbers, isFalse);
    ...
    // stays off there so the two don't double up now that the engine numbers
    // the first system too.
    expect(view().showMeasureNumbers, isFalse);
    expect(overlay()?.showBarNumbers, isTrue);
  });
