...
List<String> _scoreExtras(Score s) {
  // ⚠️ EVERY element with an id, not just notes. A span legitimately ends on a
  // REST — `c4\< d4 r4\!` in LilyPond, a wedge stop after a rest in MusicXML —
  // and indexing notes only collapsed every such endpoint to -1. Two different
  // rest-anchored spans then compared EQUAL, so the signature was quietly
  // blind to them rather than loud about them.
  final index = <String, int>{};
  var n = 0;
  for (final m in s.measures) {
    for (var v = 0; v < 4; v++) {
      for (final e in m.voiceAt(v)) {
        if (e.id != null) index[e.id!] = n++;
      }
    }
  }
