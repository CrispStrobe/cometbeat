...
✅ **X3 — Portamento family.** Effect MEMORY fixed (above). The rest stands. `1xx`/`2xx` step per tick, `3xx` target snapping,
...
🚧 **X4 — Volume slide + tremor semantics.** `Axy`'s memory rule fixed above;
⬜ it still wants an ENVELOPE measurement rather than a spectral one, and tremor
is untouched. `Axy` per-tick vs per-row, the
