...
| Instrument keymaps and zones | XM/IT keymap data is read and retained in native codec data; IT lookup is used in import resolution. | Note-to-zone selection renders for ordinary notes and sampled effect-bearing uniform/variable paths; additive and non-sample zones still use the generic fallback for tick effects. | No dedicated zone/keymap editor. | Yes. Add a zone instrument with note/velocity ranges and per-zone samples. | High |
