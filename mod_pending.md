...
| --- | --- | --- |
| Focused codec/conversion/tracker tests | PASS | 182 tests passed, including MOD, XM, S3M, IT, writers, conversion, roundtrip, tracker replay, and Advanced Tracker UI tests. |
...
| XM | Native headers, packed patterns, instruments, samples, raw PCM, and tracker metadata are retained for same-format conversion. | Native same-format output preserves the retained raw data; canonical XM is emitted from the neutral model. XM tremor, imported stereo samples, and per-sample envelopes render on the supported paths. | No native XM instrument/keymap/envelope editor. | Multi-sample instruments and several XM-specific behaviors still collapse to the neutral sample model during editing or cross-format conversion. |
...
| IT | Native headers, instruments, envelopes, sample blocks, compression, stereo/raw PCM metadata, and pattern semantics are read. | Native sample/header/instrument data is retained; patterns are semantically re-encoded rather than guaranteed byte-identical. | Yes for the supported sample/effect subset, including imported stereo samples, sample gain, per-pattern lengths, common instrument envelopes, and bounded NNA/DCT/DCA old-note actions. | No native IT instrument behavior or command editor. | Shared samples with different instrument envelopes, full keymap/zone behavior, sustain behavior in command-heavy paths, and several IT effects are not fully modeled by the renderer/editor. |
...
| Stereo samples | IT/XM right-channel PCM is parsed, written, resampled, and carried by `SampleInstrument`. | Ordinary sampled playback now preserves the native left/right image; command-heavy stereo voice paths still need coverage. | No stereo waveform editor. | Yes. Extend the per-tick stereo voice path and add an editor for both waveforms. | High |
...
| XM/IT volume and pan envelopes | Read and written in native instrument data; sampled imports carry volume/pan envelopes and loop/sustain indices. | Per-note sampled playback evaluates the active instrument envelope, including sustain hold and envelope looping on ordinary sampled paths; shared samples with different native instrument envelopes remain incomplete. | Advanced Tracker can edit channel envelopes, but not native per-instrument envelope points/flags. | Yes. Attach separate envelopes to native instrument zones and expose their flags in the editor. | High |
| IT NNA/DCT/DCA and fadeout | Native fields are read and written; imported sample voices retain NNA/DCT/DCA/fadeout metadata. | Bounded old-note actions now render for fixed sampled voices, including duplicate-note matching; full multi-voice instrument/keymap allocation and exact fade timing remain incomplete. | Not editable. | Yes. Move this state into a real per-channel voice allocator and expose it in the native editor. | High |
...
| Format-specific effects | Raw/native information is retained in some same-format paths. | XM/S3M/IT tremor and IT/S3M tempo slides now have distinct internal mappings; panbrello, MIDI, and some pattern-delay/retrigger variants remain approximated or dropped. | No format-aware command editor. | Mostly yes, command by command; MIDI hardware behavior is not necessarily renderable. | High |
