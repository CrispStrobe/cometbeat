...
| Sample loops and sustain loops | Common loop metadata is read and written; IT sustain-loop offsets and direction now survive neutral and IT roundtrips. | Ordinary loops and held-note IT sustain loops render on the normal sample path; effect-heavy sustain-loop voice behavior still needs coverage. | Basic sample editing is not a native loop editor. | Yes. Extend sustain-loop selection into every tick/voice path and expose it in the editor. | Medium |
...
| Format-specific effects | Raw/native information is retained in the song bridge for same-format exports. | XM/S3M/IT tremor, IT/S3M tempo slides, and IT/S3M panbrello now have distinct internal mappings; MIDI hardware behavior and some pattern-delay/retrigger variants remain approximated or dropped. | Native command provenance is retained, but the visible editor still exposes the generic command model. | Mostly yes, command by command; MIDI hardware behavior is not necessarily renderable. | High |
