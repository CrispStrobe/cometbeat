...
| IT | Native headers, instruments, envelopes, sample blocks, compression, stereo/raw PCM metadata, and pattern semantics are read. | Native sample/header/instrument data is retained; patterns are semantically re-encoded rather than guaranteed byte-identical. | Yes for the supported sample/effect subset, including imported stereo samples, sample gain, per-pattern lengths, and common instrument envelopes. | No native IT instrument behavior or command editor. | Shared samples with different instrument envelopes, NNA/DCT/DCA, full keymap behavior, sustain behavior, and several IT effects are not fully modeled by the renderer/editor. |
...
| XM/IT volume and pan envelopes | Read and written in native instrument data; sampled imports now carry volume/pan envelopes per imported sample instrument. | Per-note sampled playback evaluates the active instrument envelope; shared samples with different native instrument envelopes and sustain/loop semantics remain incomplete. | Advanced Tracker can edit channel envelopes, but not native per-instrument envelope points/flags. | Yes. Preserve envelope loop/sustain flags and attach separate envelopes to native instrument zones. | High |
