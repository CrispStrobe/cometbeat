...
| IT NNA/DCT/DCA and fadeout | Native fields are read and written; imported sample voices and editable zone samples retain NNA/DCT/DCA/fadeout metadata. | Ordinary native IT multi-zone playback now has a shared voice allocator with cut, note-off, fade, and duplicate matching; effect-bearing multi-zone tick paths still layer voices without the same allocator. | Advanced Tracker exposes NNA/DCT/DCA/fadeout controls in the sample/zone editor. | Yes. Share the allocator with per-tick zone rendering and match tracker-specific fade/release timing. | High |
