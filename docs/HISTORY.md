...
# Interchange fidelity — the six-codec arc (2026-08-01 → 08-03, opus) — ✅ COMPLETE

Moved here from `PLAN.md` on 2026-08-03. **Result: the cross-format corpus
signature went 10,619 → 11,717 of 21,440 (49.53% → 54.65%), 65.98% → ~72% on
the six real notation formats, and `-> lilypond` 62.4% → 82.3%. Core suite
1,960 → 3,254.** Every concept the model carries now round-trips in every
format that has notation for it.

The forward-looking remainder — what is still open and how to attack it — stays
in `PLAN.md` under "Interchange fidelity — remaining work".

- ## 📐 THE INTERCHANGE-FIDELITY ARC (2026-08-01 → 08-03, opus) — READ THIS FIRST

  One question, asked at four levels, produced everything below: **what does
  the cross-format signature NOT compare?** Anything it ignores can be
  dropped by half the codecs with every test green.

  | level | fields compared before | found |
  |---|---|---|
  | `Score` | 6 of 44 | chord symbols survived ONE format; tempo, spans, marks all partial |
  | `NoteElement` | 8 of 10 | fingering, arpeggio, notehead MusicXML-only |
  | `Measure` | 8 of 20 | barlines, pickup, mid-score tempo, mid-bar clefs |
  | metadata | 0 | `\header` was write-only; ABC `W:` invisible |

  **Result: every concept the model carries now round-trips in every format
  that has notation for it.** Suite **1,960 → 2,967**. Plain corpus signature
  **15,960/15,960**. Rich sweep **9,810 → ~10,610 / 16,080**, with the
  channels this arc added at ~2% of what remains.

  ## The bug patterns, in order of how often they recurred

  1. **A self-consistent writer+reader pair is invisible to a round trip.**
     We wrote a glissando as MusicXML `<slide>` and read `<slide>` back as a
     glissando: perfect round trip, wrong element, every third-party
     `<glissando>` dropped.
  2. **Write-only / read-only asymmetry.** LilyPond `\tempo` was emitted for
     years and never read; `\header` likewise (every export came back
     untitled); repeats were the mirror — read but never written.
  3. **CLOSE BEFORE OPEN.** A note ending one span and starting the next is
     `\!\>` and `)(`, not `\>\!` and `()`. Broke hairpins in four codecs for
     three unrelated reasons, and slurs on both sides of one codec.
  4. **POSITION decides, not order.** Whether a mark is the score's or a
     change depends on where it sits, not on whether it is the first seen.
     Hit tempo (kern, MuseScore) and clefs (MuseScore).
  5. **One pending slot where a LIST is needed.** A note carries several
     annotations, and a shared boundary note two hairpin spanners. Hit the
     ABC writer AND reader, and the MuseScore reader.
  6. **Rejecting a token DELETES the note.** LilyPond's `$`-anchored note
     regex silently dropped `c4-1`, `c'!4` and `c4:32` — the whole element,
     not just the mark.
  7. **Same element, two concepts.** MEI `<harm>` is a chord label OR figured
     bass; `<gliss>` is a glissando OR a portamento; MuseScore's Glissando
     spanner likewise. Read the CONTENT, never the tag.

  ## Measurement lessons (each cost real time)

  - **Compare by POSITION, not list index.** Comparing lyric lists by index
    reported 247,044 corruptions; keyed by note+verse it was 364. Ids are
    regenerated by every reader.
  - **A headline ratio is not comparable across harness versions.** 82% vs a
    634,043/634,044 baseline was two added target columns, not a regression —
    8/6 matched the denominator ratio exactly.
  - **Compare no more precisely than the target format can carry.** Tempo was
    compared exactly; LilyPond cannot hold a fractional `\tempo` at all.
  - **First-divergence attribution means fixing one channel RAISES another.**
    Totals are the regression signal; channel counts are a work list.
  - **The corpus catches what the suite cannot, twice over.** Two changes
    that were green on 2,800+ tests were net-negative on the corpus (kern
    local comments −204, LilyPond clefs −477) and both were reverted or
    filtered within the same sweep.
  - **Measure before choosing a filter.** 68,161 `!LO:` layout directives
    against 58 plain local comments decided how kern reads annotations.

  ## Tooling built (all in `tool/crisp_crossformat_roundtrip.dart`)

  - The rich signature now covers every score-, note- and measure-level
    channel, plus metadata — the durable half of this work.
  - **Failures tallied BY CHANNEL**, turning an unreadable 61% into a ranked
    backlog.
  - **A `<report>.current` sidecar** naming the file being processed: a
    per-2,000-file heartbeat cannot tell a slow stretch from a hang, and one
    file spun a codec for 5.5 hours before this existed.
  - Rests indexed alongside notes, since a span legitimately ends on one.

  ## 🚨 ONE THING NEEDS THE MAINTAINER, NOT AN AGENT

  **285 shipped `db.json` rows carry wrong feature data.** An ABC file is a
  TUNEBOOK and we read it as one concatenated tune, so `notes`/`bars`/
  `ambitus` count every tune in the file — `id_1-109a_Tanz_mit_mir.abc`
  (17 tunes) is recorded as 1,318 notes / 202 bars. 571 of 1,892 corpus
  `.abc` files (30%) hold more than one tune. The reader is FIXED and
  `scoreFromAbc(abc, tune: i)` + `abcTuneCount()` now address the rest; the
  DATA needs a re-index (`featgen/run_backfill.sh` → `merge_features.py` →
  re-emit the catalog). NOT done here: `db.json` is shared, locked and behind
  a publish gate. **Also a decision: should a 17-tune file be ONE row or 17?**

  ## Open gaps, in priority order

  1. **MEI mid-bar clefs** — its `<clef>` opens a layer; placing one
     mid-layer needs the layer walker to carry an onset.
  2. **LilyPond clef changes** — reverted (`ba263e5`) with the diagnosis:
     `scoreFromLilyPond` walks nodes across STAFF boundaries, so other
     staves' `\clef` commands reach the same measure builder. Needs staff
     scoping in the walker, not a patch at the point of use.
  3. **`bar` (1,695)** — measure-STRUCTURE divergence: the two sides disagree
     on how many bars exist. Untouched.
  4. **`scoreFromMidi` HANGS** on a meter finer than a sixteenth (zero-length
     bar, infinite loop). Scoped to whoever owns `scoreToMidi`; the harness
     skips that hop meanwhile — remove the skip when it lands.
  5. **MuseScore let-ring and the common/cut glyph** — BLOCKED on evidence,
     deliberately: no corpus file spells either, so inventing an encoding
     would round-trip with ourselves and mislead. Same rule the `<Harmony>`
     reader already follows.
  6. Format limits that cannot be closed: kern and ABC have no hairpin,
