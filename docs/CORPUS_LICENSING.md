...
| **Tanzsammlung Dahlhoff** (TradArchiv) ✅ **INGESTED — 672 rows** | ABC | **No copyright asserted** by the transcribers; **attribution to the source required** → Tier B | 18th-c. Westphalian dance manuscript (Staatsbibliothek zu Berlin, Mus. ms. 40182). 129,336 notes, 672/672 parse. Axis 2 spotless — source dated before 1767; the named composers that appear are Telemann d.1767, Hasse d.1783, Jommelli d.1774, Campra d.1744, and per-file `C:Urheber:` makes the death-check mechanical. Attribution surfaced in `attribution_screen.dart`. |
