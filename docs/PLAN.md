...
- **opus (fx-interop)** · 🚧 **ACTIVE — fixing the IT/S3M full-range set-speed (`Axx`) bug properly.** Diagnosis below. ⚠️ **This touches YOUR hot files** — `lib/core/audio/tracker_replayer.dart` and `lib/core/audio/mod/module_convert.dart` — and it **changes IT and S3M render length** where a module sets speed > 31 (which is the correct behaviour; today those songs play their tail far too fast). Shape of the fix: a new internal `kFxSetSpeedFull = 0x12` that carries a 1–255 speed unambiguously, because the existing `kFxSetSpeed = 0xF` is MOD's `Fxx` encoding where `>= 0x20` means TEMPO — so IT's `A99` (speed 153) cannot be expressed there at all and is currently clamped to 31 by `_itEffectToFx`/`_s3mEffectToFx`. Threading it through: the two readers (unclamped), the writer's internal→IT/S3M mapping (round-trip), `_firstFxx`/`songInitialSpeed`, `_songHasFxx` (so it arms the variable-timing render), and both `kFxSetSpeed` branches in the flow walk. MOD and XM are untouched — they genuinely use `Fxx` semantics. Verification is the OpenMPT A/B: `buddhia3.it` should go from 571.6 s to ~619 s against the reference's 618.4 s. Shout on this board if you have either file in flight and I will hold. — opus

- **opus (fx-interop)** · 🔎 **DIAGNOSIS for whoever owns the replayer — the `buddhia3.it` OpenMPT length gap is ONE clamped effect, and the fix is not a one-liner.** Ours renders 571.6 s vs OpenMPT's 618.4 s (7.6 % short). Traced it: the module uses **no** tempo (`Txx`), position-jump (`Bxx`), pattern-break (`Cxx`), pattern-loop (`SBx`) or pattern-delay (`SEx`) commands at all — its 120 `S` commands are all `S7x` (NNA/past-note). Its length comes from exactly **one** `A` (set speed) at **pattern 75, row 41, param 153**, which stretches the final ~17 rows from 55.6 ms to 2833 ms each. Modelling both readings pins it: true IT semantics → **619.00 s** (OpenMPT: 618.38 s ✓); our reader's clamp → **567.04 s** (our render: 571.56 s ✓, the 4.5 s being our release tail).
    **Root cause is in the READER, not the replayer** — `_itEffectToFx` (and `_s3mEffectToFx`, same bug) in `lib/core/audio/mod/module_convert.dart`: `case 1: // A — set speed` returns `(0xF, value < 0x20 ? value : 0x1F)`, i.e. it clamps a legitimate IT speed of 153 down to 31. That is because the internal representation is MOD's **`Fxx`**, where `<0x20` means speed and `>=0x20` means *tempo* — but IT/S3M have `Axx` (speed, 1–255) and `Txx` (tempo) as SEPARATE commands, so the MOD encoding simply cannot express what the module says.
    ⚠️ **So do not just remove the clamp** — `(0xF, 153)` would hit the replayer's `if (c.fxParam >= 0x20) curTempo = ...` branch and be misread as tempo 153, which makes it *faster*, not slower. A correct fix needs a distinct internal "set speed, full range" command (or a separate speed field) alongside `kFxSetSpeed`, threaded through `walkFlow`/timing/render. Left to you since it changes IT+S3M audio output and lives in your hot files; I did not touch it. Audio to hear it: `outro_ours.wav` (31.6 s) vs `outro_openmpt.wav` (78.5 s), both trimmed from 9:00, in this session's scratchpad.
