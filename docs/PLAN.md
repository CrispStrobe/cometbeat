...
- **opus (musicxml-tempo)** · 🚧 **ACTIVE — crisp_notation: read `<sound tempo>` on MusicXML import.** The reader takes its tempo only from `<metronome>` (the VISIBLE mark) and ignores `<sound tempo="…">` (the playback attribute), so any file that carries the latter without a printed metronome imports with no tempo at all — and plenty of exporters write exactly that. I found this landing the Song Book metadata and pinned it as a known-limitation test (`test/song_metadata_test.dart` in mus); this makes it real. Per CLAUDE.md I am working in a **sibling worktree `../crisp_notation-musicxml-tempo`** and leaving the shared `../crisp_notation` clone on `main` — nobody's path dep moves. Plan: `<metronome>` stays authoritative (it is what the score SHOWS); `<sound tempo>` is a fallback when no metronome is present, at both initial-tempo and mid-score tempo-change positions, plus writer symmetry so a round-trip does not drop it again. Then flip the known-limitation test in mus from "expect null" to the real value. — opus
...
- **opus (songbook-meta)** · ✅ **SHIPPED (idle) — per-song composer / key / tempo in the Song Book** (`a9ec19aa`). The book listed nothing but titles; `ImportedSong` now carries composer / keyFifths / tempoBpm, derived from the stored MusicXML (so it is a cache — the XML stays the source of truth) and shown as a `composer · key · ♩=bpm` subtitle. **Migration was the substantial part:** songs saved before the fields existed fill in on load from the XML that was always there, so an old book does not render as blanks; also derives on `addSong` so a fresh import shows immediately, never overwrites values already set, and leaves unparseable XML alone (a song you cannot read is still one you must be able to delete). The key label **deliberately refuses to claim a tonality** — `KeySignature` stores only `fifths`, and two sharps is D major OR B minor, so it prints the relative pair "D / Bm" rather than lying. +16 tests.
    **Also audited the whole Songbook section** — its PLANNED marker was badly stale: the collection model, persistence and the OMR import bridge all already exist. Root PLAN.md now carries per-item ✅/⬜ with `file:symbol` evidence. Genuinely still open there: **retaining the source image** so OMR can be re-run on a bad scan, and the **edit/re-run correction flow**.
