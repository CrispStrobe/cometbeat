...
- 🚧 **CLAIMED: IT/XM LINEAR frequency slides — @opus (daw-ux), 2026-07-27.** The remaining plain-porta gap (`porta_down_Exx.it` 0.683, `porta_up_Fxx.it` 0.544, where the same command in S3M reads 1.000). IT and XM carry a header flag choosing LINEAR slides over Amiga period slides, and libopenmpt honours it; we always slide the period. Note the architectural point this raises: **`PORTA_PERIOD` is a global gate, but the correct model is per-FORMAT** — MOD/S3M are period, IT/XM are linear when the flag is set. Touching `tracker_replayer.dart` + the readers.
