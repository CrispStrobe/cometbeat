...
- **opus (daw-ux)** · 🚧 **ACTIVE — replay-fidelity §6.** **Shipped since the last board update:** effect memory (per-COMMAND) · XM 16-bit loop units · S3M/IT fine porta + fine volume slides + `QUIRK_VSALL` · IT/XM linear slides · fine porta bypassing the pitch domain · **panbrello 8× too fast** · **trackers pan LINEARLY** (we panned constant-power) and panbrello depth 1/15 → 1/16 · **tremor's row-locked gate** and **XM's tremor/key-off effect numbering**. Plus `ReplayProfile`/`PitchDomain`/`PanLaw` — one profile per format, replacing booleans that had accreted one investigation at a time — and **`PORTA_PERIOD` deleted**, the slide model now `SettingsService.authenticSlides` resolved at import, because a global switch cannot serve four formats at once.
  ...
  - 🚧 **CLAIMING NOW: envelopes (volume/pan) + fadeout, the last unmeasured layer of §6.** Both directions already support them, so this is a FIDELITY question, not a feature gap. One structural risk is visible before measuring anything: `tracker_song_module._channelVolEnv` folds each instrument's envelope onto the **channel**, but XM/IT envelopes belong to the INSTRUMENT and restart on every note — a channel-level envelope has nowhere to put "restart". So the fixtures are paired: one note (shape and timing) vs two notes (does the second restart it). ⚠️ **Fixture independence matters more here than anywhere else so far:** these are written by our own writer, and if the writer encodes an envelope wrongly BOTH references read the same wrong shape, agree with each other, and our replayer reads the same file — the error cancels and the sweep goes green. So each envelope fixture also gets an arithmetic assertion on the RENDER (a ramp of a known tick length), which is independent of what we wrote. Touching `tool/`, `test/fixtures/env/`, and reading `tracker_song_module.dart`.
