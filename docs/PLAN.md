...
- 🚧 **CLAIMED: ladder X9 continued — S3M/IT FINE slides — @opus (daw-ux), 2026-07-27.** The effect fixtures are all MOD, so nothing tests the S3M/IT command set. `module_convert.dart` documents an approximation there in its own comment: *"fine slides with an 0xF nibble are approximated as a normal slide"* — but `DF2` is a fine slide of 2 ONCE per row, while the MOD `Axy` it maps to is a slide of 15 PER TICK. If that is what we do, it is not an approximation, it is roughly 45x. Same shape for `EFx`/`FFx` (fine porta) and `EEx` (extra fine). Building S3M/IT fixtures to measure it. Touching `tool/` + `test/fixtures/`; READING `module_convert.dart`.
