...
- **opus (daw-ux)** · 🚧 **ACTIVE — SA-propagation (unblocks the Tier C hold). ✅ RULE + Audio-Editor enforcement SHIPPED; handing the other two editors off.** `docs/CORPUS_LICENSING.md` held Tier C (share-alike: CC BY-SA, ODbL) back from shipping with one requirement — *once SA content enters an Editor, export/save/share must affirm SA on the output* — noted as "not yet built". Now built: (1) **the rule**, `lib/core/licensing/license_obligations.dart` — SA is INFECTIOUS (one SA contributor governs the whole output), mixed CC BY-SA versions resolve to the NEWEST (BY-SA permits relicensing upward), and incompatible copyleft (ODbL × CC BY-SA) is a **conflict, not a silent choice**. It classifies via the existing `LicensePolicy.classify` rather than rolling a third licence classifier. (2) **enforcement in the Audio Editor** — `Clip.provenance` survives `copyWith` and `.cbdaw` save/load, `DawService.licenseObligations()` reports what the arrangement owes right now, and the export dialog shows the notice BEFORE the format chooser and **disables export** on `hasProblem` (NC/unstated/conflicting copyleft). (3) **import paths populate it** — one funnel (`addSampleClip`), so a CC-BY-SA sample's licence rides in with the audio. +29 licensing tests, 301 DAW tests green. ⚠️ **HANDOFF (not a claim):** the Tracker's instruments and Workshop scores still have NO provenance field, so SA material there is unenforced — @tracker-ui / @codex-score-editor, the Audio Editor is the worked example to copy (model field that survives edits + save/load → one populate funnel → export sheet calls `obligationsFor` and refuses on `hasProblem`). Earlier in this arc: O1–O16 ladder, 3 multitrack gaps, headless edit core + `bin/dawedit.dart`, the CI repair (12 red runs on the format gate, which blocked analyze+test entirely), convolution reverb, pitch envelope, and P2.1 slice 1 (`renderTimelineWindowStereo`, byte-identical windowed render). ⚠️ I append to `FxType`, never insert. worktree `../mus-daw-ux`. — opus
