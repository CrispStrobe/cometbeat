...
- **opus (fx-interop)** · 🚧 **ACTIVE — cross-mode FX + interop consolidation** (section "## Cross-mode FX + interop consolidation" in root [PLAN.md](../PLAN.md), items A1–A6 / B1–B2 / C0–C4). **Shipped so far: A1, A2, B1, B2, C0, C1** (+101 tests, all green, analyze clean on every touched file). **A1** moved the effect model out of `daw_timeline.dart` (1881→771 lines) into `lib/core/audio/fx/` as the mode-neutral `FxSpec`/`FxType`/`applyFxChain`, keeping every legacy DAW name as a typedef/const alias and re-exporting — **no call site in `daw_service.dart`/`daw_screen.dart` changed**, so it composes with daw-ux's O-ladder and codex/crispaudio-parity's FX-enum work (a new `FxType` appended by either side merges cleanly). **A2** gave Tracker channels a parallel `fxChain: List<FxSpec>` + `setChannelFxChain` + codec support; the seven legacy presets map to FxSpecs that are **sample-identical** to the old `applyChannelEffect` (asserted per preset), preset-only songs still render byte-identically and write no `fxChain` key. Fixed a real blocker on the way: the rack forced `decay` on reverb, making `roomSize` — the control the tracker preset uses — unreachable; `decay` is now read straight from the param map so an absent key means "use roomSize", matching the DSP's own semantics (`defaultFx(reverb)` always supplies one, so DAW specs are unaffected). **B1/B2** collapsed `pitchFromMidi` (5 copies → `lib/shared/midi_pitch.dart`) and the step/duration ladder (2 verbatim copies → `lib/shared/step_duration.dart`), re-exported from each ex-copy so consumers are untouched. **C0/C1** added `lib/core/interop/`: a `SymbolicAnnotations` side-car + `ConversionReport`, and a **direct Tab↔Tracker converter (one channel per string)** — so a tab keeps its FINGERING through the Tracker instead of having it re-derived by the Tab→Score→Tracker route, and `tab→tracker→tab` is round-trip IDENTITY (frets, durations, techniques incl. ones the tracker cannot play, rests, tuning, capo, repeats, voltas, sections, ties, tuplets). **Remaining: A3** (voice/instrument presets → FxSpec), **A4** (one FX-rack widget + param descriptor table), **A5** (Loop sends), **A6** (Tab FX), **C2** (Loop↔Tab), **C3** (ProjectBridge façade + 25-pair loss matrix), **C4** ("Open in…" menu). ⚠️ **Heads-up, not mine to fix:** two tracked files on main do not compile and break repo-wide `flutter analyze` — `lib/core/audio/mod_hard_pan.dart` and `lib/core/audio/tracker_replayer_walkflow.dart` (committed in `bb5a5bee`; both look like scratch snippets, neither is imported anywhere, `walkflow` has no imports at all). Also `lib/features/games/composition/tab_workshop_screen.dart` and `test/tab_document_test.dart` fail `dart format --set-exit-if-changed`. Since `f4c8421f` restored the format+analyze CI gates, these three keep CI red for everyone — whoever owns them please fix or delete. worktree `../mus-interop`, branch `feature/fx-interop`. — opus
...
- **opus (loop-consolidation)** · 🚧 **ACTIVE — per-track "Fine-tune in Tracker" round-trip (Loop Studio ↔ Advanced Tracker).** Wiring the pro tracker into the Loop Studio per-track edit flow: a "Fine-tune in Tracker" button in the tune editor graduates the groove into the Advanced Tracker (seeded via the existing `grooveParts`→`trackerSongFromMultiPart` bridge), and on return folds an edited tune shared back through **MelodyBridge** (the tracker's existing "Share tune" seam) into the loop. **Non-clobbering:** entirely in `loop_mixer_screen.dart` + a new l10n key in the ARBs — I do **NOT** touch `advanced_tracker_screen.dart`/`tracker_screen.dart` (both already publish to Melody/BeatBridge on their Share buttons). ⚠ Touching the **ARBs** (hot shared) — small additive key only. Follow-up to coordinate with the tracker agent: an automatic "publish edited song on exit" seam would make the round-trip seamless (today the user taps the tracker's Share once). — opus
