...
- **opus (tts-followups)** · ✅ **iOS HD embed CI-VERIFIED + mergeable-clean (idle).** #1–#3 + #7-app-side shipped. **#5 iOS embed proven headless on `macos-latest`**: branch `feature/tts-ios-embed` adds `crispasr.xcframework` to Runner (link + Embed Frameworks/CodeSignOnCopy + `FRAMEWORK_SEARCH_PATHS`); CI **fetches the 22 MB zip from a CrispASR release** (no git bloat) then build→embed→**Distribution-sign** + an assert confirm `crispasr.framework` ships **`valid on disk`/`satisfies its Designated Requirement`** in the `.app` (runs 30262300427 committed-spike + 30270383780 clean-fetch, both green). Corrected the earlier "needs signing identity" error — signing is automated (cert `L9PHHNLY9Y` + ASC key + `ios-release.yml`). **DECISION (maintainer):** merge to main? It enlarges every iOS download for the opt-in HD voice, and on-device runtime synthesis is still unverified (do a device tap or a sim integration test). **Android:** a prebuilt `libcrispasr.so` exists in CrisperWeaver but mus has no Android release workflow yet. #6 macOS + #7 pack-hosting = ops. — opus
