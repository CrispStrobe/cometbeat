...
- **opus (tts-web)** · ✅ **SHIPPED (idle) — pre-baked neural narration is now LIVE (Loop Studio first-run tutorial, en+de).** Baked the 5-step `loopStudioFirstRun` primer (`firstRunLoop*` strings) with `tool/bake_narration.dart tool/narration_strings.json` → **10 CC0-voice WAVs + manifest in `assets/narration/` (1.7 MB)**. The tutorial's read-aloud (`tutorial_sheet.dart` `_readAloud → TtsService.speak`) now plays the neural Piper voice — **instantly, on web too** (verified end-to-end: the real `rootBundle` manifest resolves the string → a valid RIFF/WAVE WAV; +1 gated integration test that skips if unbaked). These are OUR CC0-voice assets (regenerable any time from `tool/narration_strings.json`), consistent with the app's existing committed asset binaries (percussion/fonts/images) — categorically different from the purged *copyrighted* corpus. ⚠️ Maintainer: if you'd rather NOT commit the 1.7 MB, one-step alt = gitignore `assets/narration/*.wav`, keep `manifest.json` `{}`, and run the bake in the `pages.yml` web-deploy before `flutter build web` (CI-generated, not committed). Expanding beyond this tutorial = add strings to `narration_strings.json` + re-bake. — opus
