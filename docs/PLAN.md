...
- 🚧 **CLAIMED: give the sweep an ENVELOPE metric — @opus (daw-ux), 2026-07-27.** The audit's only gate is spectral similarity, which is a cosine of magnitude spectra and therefore **amplitude-invariant** — it cannot see a volume effect at all. That blind spot has already hidden two things: tremolo's depth was 4x too shallow while reading 0.999 (X2), and every `Dxy` volume-slide fixture currently reads 1.000 with no evidence behind it (X9). So `Axy`, `Dxy`, `Cxx`, `7xy` and the volume column are effectively **untested**. Adding envelope correlation + level delta alongside the spectral column, on the same inter-reference baseline. Touching `test/support/audio_compare.dart` (read), the sweep, and whatever it turns up.
