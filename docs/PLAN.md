...
- **opus (daw-ux)** · 🚧 **ACTIVE — replay-fidelity §6, and a LADDER RE-AUDIT of my own, 2026-07-28.** Prompted by the workstation re-audit above: I checked §6's status lines against the code instead of memory and found **my own X3/X4 write-up had been stale for a day**. The `8a2c2d52` clobber reverted the PLAN section along with the fix; when I restored it I verified the four `lib/` files and the test against the reference players and **never checked the docs**, so §6 read "X3/X4 DIAGNOSED, NOT FIXED" while the fix was live. That is how X2 got built twice. **Restoring code is not restoring a commit — diff the whole clobber.** Restored, and X2/X3/X4/X9/X10's status lines re-verified by symbol. **Shipped since the last board update:** effect memory (per-COMMAND), XM 16-bit loop units, S3M/IT fine porta + fine volume slides + `QUIRK_VSALL`, IT/XM linear slides, fine porta bypassing the pitch domain, **panbrello 8× too fast**. Plus `ReplayProfile`/`PitchDomain` (one profile per format, replacing three accreted booleans) and **`PORTA_PERIOD` deleted** — the slide model is now `SettingsService.authenticSlides`, resolved at import, because a global switch cannot serve four formats. ⚠️ **Three metric blind spots closed, each found only after the last:** spectral is amplitude-invariant (hid tremolo's 4× depth) → envelope metric; no envelope metric (hid volume slides) → added; **mono downmix (hid ALL panning)** → pan-trajectory metric. 🔶 **In progress:** the references pan LINEARLY, we pan constant-power — a per-profile `PanLaw`, `native` keeping constant-power so the app's own mixer is untouched. ⬜ **Owner's call, blocking 4 fixtures:** `TrackerCell.volume` is both the 0..64 channel volume and a 0..1 ghost-note gain; splitting them changes song semantics, not just import. ⚠️ **`flutter test` default concurrency dies with "Cannot add event while adding stream" under load on this box — use `--concurrency=3..6`.** — @opus (daw-ux)
