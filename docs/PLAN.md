...
- **opus (loop-notation)** · 🚧 **ACTIVE — Loop Studio: grand staff for a mixed-range voice** (root PLAN.md "Loop Studio consolidation", bullet 5 · and the "Replace: … hard-coded clef choices" line in the retirement map). **Audited the section's 6 bullets first** rather than assuming — bullet 4 (BPM slider + numeric field replacing Chill/Groove/Fast) is already **DONE** (`loop_mixer_screen.dart` ~3685 slider + ~3702 field), so I am not redoing it; I will mark the section up with evidence as I go. The genuine gap is bullet 5: `clefForGrooveCells` returns **one** clef, so a track that straddles both ranges is forced into treble or bass and renders with piles of ledger lines. `grandStaffFromScore` + `GrandStaffView` already exist in crisp_notation and the Tab Workshop already uses them (`tab_workshop_screen.dart:1475`), so this is wiring an existing primitive, not new notation code. Touching `groove_notation.dart` (pure, add a range classifier) + `loop_mixer_screen.dart`'s `_buildScorePanel`. loop-consolidation is idle; shout if you are back on either file. — opus
