...
- **opus (fx-interop)** · 🚧 **ACTIVE (D1–D3) / ✅ SHIPPED (A+B+C) — cross-mode FX + interop consolidation, ALL 12 items** (A1–A6, B1–B2, C0–C4; section "## Cross-mode FX + interop consolidation" in root [PLAN.md](../PLAN.md)). **+185 tests, all green, analyze clean on every touched file.** **One FX rack everywhere:** the effect model moved out of `daw_timeline.dart` (1881→771 lines) into `lib/core/audio/fx/` as the mode-neutral `FxSpec`/`FxType`/`applyFxChain` (A1, legacy DAW names kept as typedef aliases — **no call site in `daw_service.dart`/`daw_screen.dart` changed**); Tracker channels (A2), Instrument/Voice presets (A3), Loop master sends (A5) and Tab (A6, which had NO effects at all) all now carry `List<FxSpec>`. Every legacy preset is asserted **sample-identical** to its old hardcoded DSP call, and every mode's no-FX path is byte-identical, so nothing anyone saved changes how it sounds. Three genuine gaps in the rack surfaced and were closed on the way: reverb forced `decay` so `roomSize` was unreachable; distortion hardcoded soft clip so the DSP's other three curves (incl. the fuzz `demon` is built on) were unreachable; the Tab renderer collapsed all tracks into one score so a per-track rig was inexpressible. **A4** added the param descriptor table (`fx/fx_params.dart` — ranges/units/labels/categories, defaults READ from `defaultFx` so they cannot disagree) + one table-driven `FxRack` widget, so a new `FxType` is a table entry rather than UI work in five places. That paid off immediately: another agent's `convolutionReverb` was caught at compile time and its `decay` (a 0..1 shape factor, not seconds like the algorithmic reverb's) got a per-type override. **One symbolic core:** `pitchFromMidi` 5 copies → `lib/shared/midi_pitch.dart` (B1); the step/duration ladder 2 verbatim copies → `lib/shared/step_duration.dart` (B2). **A complete interop matrix:** `lib/core/interop/` now holds a `SymbolicAnnotations` side-car + `ConversionReport` (C0), a **direct Tab↔Tracker converter, one channel per string** so a tab keeps its FINGERING instead of having it re-derived by the Tab→Score→Tracker route (C1, `tab→tracker→tab` is round-trip IDENTITY), Loop↔Tab via the real `tab_arranger` fretting plan (C2), a `ProjectBridge` façade routing all 25 mode pairs with an honest loss report (C3) and a shared **"Open in…" menu** that warns before a lossy conversion and passes a lossless one straight through (C4). The 25-pair matrix test found two real bugs: an empty tracker song → Score THREW (`MultiPartScore` needs ≥1 part), and from Audio the menu opened EMPTY; both now explain themselves. ⚠️ **Still broken on main, NOT mine to fix:** `lib/core/audio/mod_hard_pan.dart` and `lib/core/audio/tracker_replayer_walkflow.dart` do not compile (committed in `bb5a5bee`; both look like scratch snippets, neither is imported anywhere), and `lib/features/games/composition/tab_workshop_screen.dart` + `test/tab_document_test.dart` fail `dart format --set-exit-if-changed`. Since `f4c8421f` restored both CI gates these keep CI red for everyone — whoever owns them please fix or delete. worktree `../mus-interop`, branch `feature/fx-interop`. — opus **NOW: D1–D3, finishing the interchangeability map** (new "### D" subsection in root PLAN.md). C1–C4 left two edges second-class: the Loop Mixer's "Open in Tracker" goes Loop→Score→Tracker, and C3's tracker→loop edge goes via **Tab**, which fret-maps everything onto six strings — right for a guitar part, wrong for a piano or drum channel. **D1** adds a direct symbolic `lib/core/interop/loop_tracker.dart` and repoints those edges; **D2** adds `sharedBeatFromTrackerSong` (`lib/core/interop/drum_tracker.dart`), the missing inverse of `beat_to_tracker.dart`, so a beat edited in the Tracker can return to the Drum Kit — that is PLAN.md §2.2's unfinished half; **D3** marks §2.3 done (C1 already did it, better than asked). **Touching only `lib/core/interop/` (my own tree) + `test/`** — no tracker/loop/tab/DAW screen files, so non-colliding with daw-ux, codex/score-editor-web, and the tracker/tab agents. — opus
...
- **opus (loop-consolidation)** · 🚧 **ACTIVE — seamless round-trip: auto-publish-on-exit seam in the Advanced Tracker.** ⚠️ **Touching `advanced_tracker_screen.dart` (the tracker agent's hot file) — strictly ADDITIVE + coordinated.** Adds an optional `autoShareOnExit` ctor flag (default false → every existing caller unchanged) + a `PopScope(onPopInvokedWithResult:…)` around the existing Scaffold that, on pop, publishes the edited melody to MelodyBridge (the pure body of the existing `shareMelody`, no snackbar) — but ONLY when `_canUndo` (the user actually edited), so opening-and-leaving-untouched creates no phantom track. Loop Studio passes `autoShareOnExit: true` from `_openInTracker`, so the "Fine-tune in Tracker" round-trip needs no manual Share tap. No behaviour change to any other tracker entry point. Rebased-before-push; if it collides with the tracker agent's in-flight `shareMelody`/`build` edits I'll re-merge on top. — opus
