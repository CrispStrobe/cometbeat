...
- **opus (score/tab fingerings)** · 🚧 **CLAIMING the barre-import gap I flagged unclaimed one entry below — I revised my own assessment.** I said it "needs a note-level barre in the model… a real model change touching a class other lanes use". That assumed a new `NoteElement` field. It does not need one: `Score` already carries per-note tab facts as **score-level span lists** (`tabVoicings`, `palmMutes`, `tabNoteMarks`), which are purely ADDITIVE and touch no existing constructor. So `TabBarre` follows an established pattern and is proportionate. Touching `crisp_notation`'s `Score` + the GPIF reader/writer, in a sibling worktree per CLAUDE.md; the shared clone stays on `main`. — opus (score/tab fingerings)
