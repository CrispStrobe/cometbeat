...
- **opus (fx-interop)** · ✅ **SHIPPED (idle) — cross-mode FX + interop, A1–A6 / B1–B2 / C0–C4 / D1–D3.** **+229 tests, all green, analyze clean.** **FX:** one `FxSpec` rack in `lib/core/audio/fx/` shared by Audio/Tracker/Instrument/Loop/Tab (Tab had none before), every legacy preset asserted **sample-identical** to its old hardcoded DSP call and every no-FX path byte-identical; plus a param descriptor table + one table-driven `FxRack` widget so a new `FxType` is a table entry rather than UI work in five places. **DRY:** `pitchFromMidi` 5→1, the step/duration ladder 2→1. **Interop (`lib/core/interop/`):** side-car + loss report (C0), Tab↔Tracker one-channel-per-string (C1), Loop↔Tab (C2), `ProjectBridge` over all 25 mode pairs (C3), `OpenInMenu` (C4), **direct symbolic Tracker↔Loop** replacing the old Score/Tab detours (D1), and **Tracker→DrumKit** completing PLAN.md §2.2 (D2). **⇒ PLAN.md §2 "Ecosystem Interchangeability" is now DONE** (all three items; §2.3 was delivered by C1). Bugs found by the round-trip tests and fixed: loop rests were swallowed by the preceding note (needs an explicit `keyOff` + `noteRuns`); **`beat_to_tracker.dart` silently dropped a beat's SWING** (pre-existing, one-line fix); an empty tracker song → Score threw; the Open-in menu opened empty from Audio; a parallel agent's new `convolutionReverb` was caught at compile time. **⚠️ WHAT IS LEFT — UI WIRING, deliberately not done from here:** no screen hosts `FxRack` or `OpenInMenu` yet. The models are all in place (`TrackerChannel.fxChain` + `setChannelFxChain`, `TabTrack.fxChain`, `LoopEngine.masterFxChain`, `ProjectBridge`), so each host is a small independent change — **left to the agents who own tracker/loop/tab/DAW screens** rather than editing their hot files from this branch. ⚠️ **Still red on main, not mine:** `lib/core/audio/mod_hard_pan.dart` + `lib/core/audio/tracker_replayer_walkflow.dart` do not compile (`bb5a5bee`, scratch snippets, nothing imports them) and `tab_workshop_screen.dart` + `test/tab_document_test.dart` fail the format gate — these keep the restored CI gates red for everyone. worktree `../mus-interop`. — opus
