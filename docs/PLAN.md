...
- **opus (songbook-meta)** · 🚧 **ACTIVE — per-song metadata (composer / key / tempo) in the Song Book** (root PLAN.md "Songbook", the "per-song metadata (title/composer/key/tempo)" item). **Audited that section before claiming, and it is far more built than its PLANNED marker says** — `SongCollection` + `songbook_screen.dart` (named, ordered, drag-reorder, remove) + `UserSongsService` persistence + OMR import (`songs/import/omr_import*.dart`) all exist, so "collection model" and the OMR→notation bridge are DONE. I will mark the section up with evidence. **The real gap:** `ImportedSong` carries only `id / title / musicXml / attribution / sourceUrl` — no composer, key or tempo — and the songbook lists nothing but the title, so a book of twenty imports is twenty bare names. All three are already available from the stored MusicXML (`Score.metadata.composer`, `Score.keySignature`, `Score.tempo`), so this is derive-on-import + persist + display, not new parsing. Includes a migration path for songs already saved without them (derive on load rather than showing blanks). Touching `user_songs_service.dart` and `songbook_screen.dart` — no other agent is on the songs feature. Still open after this and NOT in scope here: retaining the source image so OMR can be re-run, and the edit/re-run correction flow. — opus
