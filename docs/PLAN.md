...
- **opus (notename-contract)** · 🚧 **ACTIVE — lock the note-naming contract with property tests.** `noteName` (English / German-H / solfège / localized) backs dozens of reading games but only `B=H` is spot-checked. Add `test/note_name_contract_test.dart`: every Step names non-empty in every explicit style; German-H renames ONLY B→H; solfège is the fixed-do syllables; the localized (auto) path equals English in en and German-H in de. Test-only, new file — zero collision.
