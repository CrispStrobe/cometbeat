...
- **opus (loop-solo)** · 🚧 **ACTIVE — per-track SOLO in Loop Studio** (root PLAN.md, "Loop Studio UX contract": *"Each track exposes play/mute/solo, level, instrument, record/overdub, and an Edit button"*). **Audited the rest of that section first** and it is in better shape than its IN-PROGRESS marker suggests — bullets 1 (captured layers are symbolic `MelodicPattern`/`DrumRowsPattern`, not baked stems), 2, 4, 5 and 6 all check out, and `PerformScreen` is already out of navigation (referenced only by its own file + tests), so bullet 3's "remove redundant navigation entries" is effectively done too. I will mark the section up with evidence. **The one real gap is solo:** `grep -i solo lib/core/audio/loop_engine.dart` returns nothing — `enabled` is the only gate, so you can mute-by-disabling but cannot say "just this one". Adding `soloed` with the same semantics the Tab mode already uses (`audibleTracks` in `tab_document.dart`: if anything is soloed, only the soloed ones sound; else every non-muted one) + a per-track solo control + tests. Touching `loop_engine.dart` and `loop_mixer_screen.dart`; loop-consolidation is idle. (Also checked: `loopMixerSolo` is "Solo **pad**" — a lead pad, correctly named, not a mixer solo.) — opus
