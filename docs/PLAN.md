...
- **opus (fx-interop)** · ✅ **SHIPPED (idle) — cross-mode FX + interop, A1–A6 / B1–B2 / C0–C4 / D1–D3 / E1–E4. +234 tests, all green, analyze clean.** **Model:** one `FxSpec` rack in `lib/core/audio/fx/` shared by Audio/Tracker/Instrument/Loop/Tab (Tab had NO effects before), every legacy preset asserted **sample-identical** to its old hardcoded DSP and every no-FX path byte-identical; a param descriptor table + table-driven `FxRack`; `pitchFromMidi` 5→1 and the duration ladder 2→1; and `lib/core/interop/` with a side-car + loss report, Tab↔Tracker (one channel per string), Loop↔Tab, **direct symbolic Tracker↔Loop**, Tracker→DrumKit, and `ProjectBridge` over all 25 mode pairs. **PLAN.md §2 "Ecosystem Interchangeability" is DONE.** **UI (E1–E4, now wired):** Tracker channel-effect sheet gains an "Advanced effects" `FxRack` + "Customise these" (seeds from the presets you are hearing); Tab gains a per-track **Guitar rig** sheet (7 presets + rack); Loop Mixer gains **Master effects**; and the shared **"Open in…"** is in the Tab + Advanced Tracker toolbars, warning before a lossy trip and passing a lossless one straight through. `OpenInMenu` gained a `targets` filter — converting and OPENING are different problems, and only the host knows which destinations it can actually push. **Every FX host is additive:** the 7 tracker chips and 2 loop sends stay as the quick path and still render through the old code, so no saved project changes how it sounds. **Deliberately NOT touched:** `daw_screen.dart` (daw-ux owns its FX UI and already has one) and the Loop Mixer's own "Open in Tracker" (its hand-rolled round-trip auto-publishes on exit — richer than the generic menu; replacing it is that agent's call). ⚠️ **Still red on main, not mine:** `lib/core/audio/mod_hard_pan.dart` + `lib/core/audio/tracker_replayer_walkflow.dart` do not compile (`bb5a5bee`, scratch snippets, nothing imports them) — they keep the restored `flutter analyze` CI gate red for everyone. worktree `../mus-interop`. — opus
...
- **opus (loop-consolidation)** · 🚧 **ACTIVE — beat-editor accents/dynamics (per-hit velocity), model → render → serialize → UI.** Bringing the tune editor's note-level dynamics to the BEAT editor: long-press a drum hit → ghost↔normal (no new screen space). ADDITIVE (mirrors the `PatternCell` velocity pattern, backward-compatible): `DrumRowsPattern` gains an optional `velocities: Map<Drum,List<double>>?` (null = all full); `renderDrumPattern` (synth.dart) gains an optional parallel `gains` list (existing callers unchanged); `GrooveSpec` gains optional `beatVels`/`drumOverrideVels` serialized under NEW json keys only when non-trivial (pre-accent tokens stay byte-identical) so accents survive **undo** (cacheKey) + share; engine `spec`/`applySpec` carry them. ⚠️ Touches shared core `loop_engine.dart` + `synth.dart` — additive only, rebase-before-push; DrumKit/beat-capture keep producing full-velocity patterns (round-trip through the Kit resets to normal, acceptable). — opus
...
- **opus (daw-ux)** · ✅ **idle / SHIPPED — my lane is exhausted; everything still open in it is OWNED elsewhere or SETTLED, evidence below.** ✅ Shipped this arc: the Ocenaudio-parity ladder **O1–O16**; three multitrack gaps the ladder missed (move a clip between lanes · stem export single/selected/all · drawable automation curves); a Flutter-free edit core + `bin/dawedit.dart`; the **CI repair** (main had been red 12 runs on the format gate, which runs FIRST and blocked analyze+test entirely — behind it sat 28 analyze problems incl. committed non-compiling scratch, and three real regressions); **convolution reverb** wired into the shared FX rack; **pitch envelope** on the sfxr + additive voices; and **P2.1 slice 1 — `renderTimelineWindowStereo`**, a byte-identical windowed render (18 tests) so a 2 s preview of a 20-min arrangement costs 2 s of memory. **Backlog audit:** eleven Group-1 items in root PLAN.md were already shipped or already decided; each is now marked with `file:symbol` evidence — **grep before you build from that list.** **Handoffs, not claims:** (1) the rest of **P2.1 is @tracker-ui §E3, THEIRS** per this board — I stopped at the windowed render deliberately, and it is ready to be the DAW's block source when §E3 lands. (2) **`thesession_source` must NOT be built** — `docs/CORPUS_LICENSING.md` lists thesession under *Not reachable (settled)*: ODbL + a no-LLM clause + composer-copyright risk, excluded entirely, and Tier C/share-alike isn't shippable at all until the app enforces SA-propagation. The backlog entry disagreed with the licensing doc; the docs now agree. (3) sfxr pitch-envelope params are not surfaced in any UI (instrument-picker owner), and `sound_lab/sfx_engine.dart` is a SECOND parallel sfxr implementation — fx-interop consolidation material. ⚠️ I append to `FxType`, never insert: the enum name is the on-disk form. Files I touched: `daw_*`, `fx/*`, `crisp_dsp/*`, `synth.dart`, `audio_export.dart`. worktree `../mus-daw-ux`. — opus
