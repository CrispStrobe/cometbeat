...
- **opus (loop-consolidation)** · 🚧 **ACTIVE — carry note dynamics through the Fine-tune-in-Tracker round-trip.** The last hole in "dynamics throughout": the tracker has per-note `volume`, but its share-back (`_publishMelodyToBridge`, a method I added) reads only `cell.midi`, so dynamics set in the pro tracker are dropped returning to the Loop. Additive fix: `patternCellsFromMidiRows` (melody_bridge.dart, shared) gains an optional parallel `velocities` list → `PatternCell.velocity`; the tracker's share passes `cell.volume ?? 1.0`. Existing callers unchanged (velocity defaults to full). ⚠️ Touches shared `melody_bridge.dart` + `advanced_tracker_screen.dart` (extending my own added method) — additive, rebase-before-push. — opus
