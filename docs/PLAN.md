...
- **opus (daw-ux)** · 🚧 **ACTIVE — backlog audit + pickups; ⚠️ P2.1 (real-time streaming DAW engine) needs a coordination call before anyone starts, see below.** ✅ pitch envelope SHIPPED (sfxr + additive voices, persisted in the instrument codec; the additive core keeps its original fixed-pitch expression verbatim so the tracker's byte-identical render gate still holds — the phase accumulator runs only when an envelope is present). ✅ convolution reverb wired into the shared FX rack. **Backlog audit:** nine Group-1 items in root PLAN.md were already shipped and have been marked ✅ with `file:symbol` evidence (the whole Audio-FX block bar one, plus the starter-module generator) — please grep before building from that list. **Open + unclaimed after the audit:** `thesession_source.dart` (verified absent) and **P2.1**, which is the biggest remaining DAW gap: `renderTimeline` is still whole-buffer offline, so faders/automation only apply on the next bake and nothing is live-playable. P2.1 overlaps the Tracker arc's §E3 `flutter_soloud` claim AND the Tracker just shipped its own streaming mixer (Renderer v2.1), so there is probably ONE engine to share rather than two — I have NOT started it. ✅ SHIPPED so far: the whole Ocenaudio-parity ladder O1–O16, three multitrack gaps the ladder missed, a headless edit core + CLI, and the CI repair.** (1) **O1–O16 complete** (see "## Audio Editor — Ocenaudio-parity ladder" below) — normalize/invert/remove-DC/trim-silence/amplify · crop+silence range · generate · zoom · loop-the-selection · clip stats · 6 more biquad FX + a new phaser · level meters · markers · mic record · spectrogram · AIFF + Ogg-Vorbis import. Only FLAC/OGG **export** is out, and only because glint has no FLAC encoder — Opus is a vendoring pass away, handed over in `docs/GLINT_ENCODER_HANDOVER.md` (unclaimed) with the Dart FFI side already on main and degrading to null. (2) **Beyond the ladder** (it was written against a single-file editor, so it never asked for these): move a clip BETWEEN lanes (`moveClipToTrack` + drag + picker) · per-track **stem export**, single/selected/all, stems skip the master limiter so they sum back to the mix · **drawable** automation curves (`automation_curve_editor.dart`). (3) **`lib/core/audio/daw_edits.dart` + `bin/dawedit.dart`** — the edit maths is Flutter-free and CLI-drivable, so every op is unit-testable headlessly AND audible on a real WAV. (4) **CI repair** — `main` had been red 12 runs on the format gate, which runs FIRST and so blocked analyze+test entirely; behind it sat 28 analyze problems (incl. committed non-compiling scratch) and three real regressions (4×-short tab durations, custom progressions decoding I–V as I–iii, a dangling `arrange_loops→tracker`). (5) **Convolution reverb wired into the shared FX rack** — it was implemented+tested but Voice-Lab-only. ⚠️ I append to `FxType` (never insert): the enum name is the on-disk form. Files I touch: `daw_*`, `fx/*`, `crisp_dsp/*`, `audio_export.dart`. worktree `../mus-daw-ux`. — opus
