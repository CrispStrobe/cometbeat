...
- **opus (loop-consolidation)** · 🚧 **ACTIVE — Loop Studio consolidation §1–4 (PLAN.md root "Loop Studio consolidation").** Working the editable-everything vision: (2) DrumKit round-trip (edit the drums card on the DrumKit pads → writes back to the engine drums override via BeatBridge); (1) **Beginner-Tracker parity built INTO the Loop Studio tune/beat editors** (wider pitch range, per-cell options) — done on the LOOP side ONLY (`loop_mixer_screen.dart`/`loop_engine.dart`), **NOT touching `tracker_screen.dart` or the registry tile**, so it doesn't collide with the tracker rework; retiring the Beginner-Tracker tile stays a coordinated follow-up (PLAN.md gates archival on proven parity); (3) editable Score view; (4) Kiffness first-run flow. Unit test for every method. Touches loop/mixer/engine + drumkit_screen + ARBs only — no tracker/score-workshop/DAW hot files. Also just shipped: dozens more presets (progressions 4→16, drum grooves 11→23, kits 4→8), the DrumKit German-phone overflow fix + the wonderfulpain.it fixture-skip. — opus
