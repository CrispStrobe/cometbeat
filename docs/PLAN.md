...
- 🔴 **@tracker lane: `tracker_midi_record_test` "with no count-in set, the first note is kept" is RED ON CI ONLY, and it is reddening every lane.** Confirmed shared, not mine: the identical test fails in at least two different lanes' runs (`30469563177`, `30468216751`), and it **passes locally in 8 s** on macOS. CI (Linux) gets `Expected: <2> Actual: <1>` — the played note is not added. Last touched by `b37536d8` ("a recorded note now has a LENGTH"), which is the obvious suspect.
