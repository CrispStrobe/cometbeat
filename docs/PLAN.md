...
- **opus (daw-ux)** · 🚧 **ACTIVE — replay-fidelity §6.** **Shipped since the last board update:** effect memory (per-COMMAND) · XM 16-bit loop units · S3M/IT fine porta + fine volume slides + `QUIRK_VSALL` · IT/XM linear slides · fine porta bypassing the pitch domain · **panbrello 8× too fast** · **trackers pan LINEARLY** (we panned constant-power) and panbrello depth 1/15 → 1/16 · **tremor's row-locked gate** and **XM's tremor/key-off effect numbering** · **pan envelopes dropped by a dispatch fast-path** and **IT's SIGNED pan envelope read as unsigned**. Plus `ReplayProfile`/`PitchDomain`/`PanLaw` — one profile per format, replacing booleans that had accreted one investigation at a time — and **`PORTA_PERIOD` deleted**, the slide model now `SettingsService.authenticSlides` resolved at import, because a global switch cannot serve four formats at once.
  ...
  - 🚧 **CLAIMING NOW: fixture independence, the cheap half.** Every fixture in `test/fixtures/` is written by OUR writer, so a writer bug is baked into every A/B that uses it — and five both-directions bugs turned up in this audit, so that is not hypothetical. The expensive version (author the same content with an external tool) needs tooling I have not confirmed exists. The cheap version is the one that has already paid twice: **check the fixture against ARITHMETIC before comparing anything to us.** It caught every IT envelope fixture rendering silent (a 1-based keymap filled with 0), and the numbering table caught XM `15h`. The gap left is the NOTE table: nothing asserts what period our writer puts on disk for an authored note, and ProTracker's table is a published constant (`C-2` = 428). If that were wrong every exported MOD would be detuned and no A/B could see it, because our reader would read the same wrong value back. Touching a new test file only. — @opus (daw-ux)
