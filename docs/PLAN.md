...
- 🚧 **CLAIMED: `ReplayProfile` / `PitchDomain` refactor — @opus (daw-ux), 2026-07-27.** Three per-format booleans have accreted onto `TrackerChannel` in three separate passes (`protrackerMemory`, `volumeSlideAllTicks`, `linearSlides`), each threaded through the same ~8 `ReplayVoice` sites — and I missed a site once, and the two vibrato branches silently disagreed about direction because the same `period ? … : …` ternary is written out at four call sites. Replacing them with ONE profile per source format plus a `PitchDomain` that owns its own arithmetic. **Behaviour-neutral first** (verified by the sweep), then a separate commit deletes the `PORTA_PERIOD` gate. Key enabler: **authored songs are their own profile** — they are not MOD imports, which is why flipping that gate has felt risky. Touching `tracker_engine.dart`, `tracker_replayer.dart`, `tracker_song_module.dart`.
